...
abstract class MangaBox(
    ...
    override fun popularMangaRequest(page: Int): Request {
        Log.d("TEST", "popularMangaRequest 1")
    ...
    override fun latestUpdatesRequest(page: Int): Request {
        Log.d("TEST", "latestUpdatesRequest 1")
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        Log.d("TEST", "mangaDetailsParse 1")
        return SManga.create().apply {
            Log.d("TEST", "mangaDetailsParse 2")
            document.select(mangaDetailsMainSelector).firstOrNull()?.let { infoElement ->
                Log.d("TEST", "mangaDetailsParse 3")
                title = infoElement.select("h1, h2").first()!!.text()
                Log.d("TEST", "mangaDetailsParse title $title")
                author =
                    infoElement
                        .select("li:contains(author) a, td:containsOwn(author) + td a")
                        .eachText()
                        .joinToString()
                Log.d("TEST", "mangaDetailsParse author $author")
                status =
                    parseStatus(
                        infoElement.select("li:contains(status), td:containsOwn(status) + td").text(),
                    )
                genre = infoElement
                    .select("div.manga-info-top li:contains(genres)")
                    .firstOrNull()
                    ?.select("a")
                    ?.joinToString { it.text() } // kakalot
                    ?: infoElement
            ...
            Log.d("TEST", "mangaDetailsParse thumbnail_url $thumbnail_url")
    ...
    private fun Element.selectDateFromElement(): Element {
        ...
        return this.select(defaultChapterDateSelector).lastOrNull() ?: this
            .select(
    ...
    override fun chapterFromElement(element: Element): SChapter =
        SChapter.create().apply {
            ...
            date_upload = parseChapterDate(element.selectDateFromElement().attr("title"))
            Log.d("TEST", "chapterFromElement date_upload ${element.selectDateFromElement().attr("title")}")
            Log.d("TEST", "chapterFromElement date_upload $date_upload")
        }

    private fun parseChapterDate(date: String): Long =
        try {
            dateFormat.parse(date.replace(Regex("[\"“”‘’]"), "").trim())
        } catch (e: ParseException) {
            Log.e("TEST", "Failed to parse: $date", e)
            null
        }?.time!!
