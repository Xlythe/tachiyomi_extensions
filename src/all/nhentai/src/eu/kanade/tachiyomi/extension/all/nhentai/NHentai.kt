...
open class NHentai(
    ...
    private var displayFullTitle: Boolean = when ("full") {
        "full" -> true
        else -> false
    }
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        return SManga.create().apply {
            ...
            thumbnail_url = "https://$cdnUrl/${data.pages.firstOrNull()?.thumbnail ?: ("galleries/${data.media_id}/1t.webp")}"
    ...
    override fun pageListParse(document: Document): List<Page> {
        val data = document.getHentaiData()
        val cdnUrl = "i1.nhentai.net"

        return data.pages.mapIndexed { i, image ->
            Page(
                index = i,
                imageUrl = "https://$cdnUrl/${image.path}",
            )
        }
    }

    private fun Document.getHentaiData(): Hentai {
        val script = selectFirst(hentaiSelector)?.data() ?: throw Exception("nHentai SvelteKit data not found")
        val responseJson = json.parseToJsonElement(script).jsonObject
        val bodyStr = responseJson["body"]?.jsonPrimitive?.content ?: throw Exception("Body not found in SvelteKit metadata")
        return json.decodeFromString<Hentai>(bodyStr)
    }
